...
const StaticBannerSection = ({ config, updateNested, isUpdating }) => { // Accept config, updateNested, isUpdating as props
  const bannerInputRef = React.useRef(null);

  const handleUpdateField = (field, value) => {
    console.log(`StaticBannerSection: Updating field: ${field}, value: ${value}`);
  ...
  const handleUpload = async (event) => {
    ...
    try {
      ...
      handleUpdateField('static_banner_image_url', imageUrl);
    } catch (error) {
  ...
  const handleRemoveBanner = () => {
    handleUpdateField('static_banner_image_url', '');
    showError("Static banner image removed from local configuration.");
  };
