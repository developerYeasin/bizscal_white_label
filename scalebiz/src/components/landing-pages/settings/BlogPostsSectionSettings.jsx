...
const BlogPostsSectionSettings = ({ component, index, updateNested, isUpdating }) => { // Accept updateNested, isUpdating as props
  const fileInputRefs = React.useRef({});

  const handleUpdateField = (field, value) => {
    console.log(`BlogPostsSectionSettings: Updating field: ${field}, value: ${value}`);
  ...
  const handleUpdatePostField = (postId, field, value) => {
    console.log(`BlogPostsSectionSettings: Updating post ${postId} field: ${field}, value: ${value}`);
  ...
  return (
    <CollapsibleCard
      title={`Blog Posts Section (ID: ${component.id})`}
    >
      <div className="grid gap-4">
        <div>
          <Label htmlFor={`blog-posts-title-${component.id}`}>Section Title</Label>
          <Input
            id={`blog-posts-title-${component.id}`}
...
export default BlogPostsSectionSettings;