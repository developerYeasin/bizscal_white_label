...
import { useDebounce } from "@/hooks/use-debounce.js"; // Import useDebounce
...
const AssignCategoriesDialog = ({ isOpen, onClose, selectedCategoryIds, onSelectCategories }) => {
  ...
  }, [availableCategories, debouncedSearchTerm]);
  ...
  const isAllFilteredSelected = filteredCategories.every(category =>
  ...
  const handleSelectAllToggle = () => {
    if (isAllFilteredSelected) {
      ...
      const newSelection = tempSelectedCategoryIds.filter(id =>
        !filteredCategories.some(category => String(category.id) === id)
      );
      setTempSelectedCategoryIds(newSelection);
    } else {
      // Select all filtered categories that are not already selected
      const newSelection = [
        ...tempSelectedCategoryIds,
        ...filteredCategories.map(category => String(category.id)).filter(id => !tempSelectedCategoryIds.includes(id))
      ];
      setTempSelectedCategoryIds(newSelection);
    }
  };
          ...
          <ScrollArea className="flex-1 h-0 px-4">
            <div className="py-4">
              ...
              {isLoading ? (
                ...
                <p className="text-center text-muted-foreground">No categories found matching your search. Click '+' to create one.</p>
              ) : (
