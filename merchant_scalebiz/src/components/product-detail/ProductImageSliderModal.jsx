...
const ProductImageSliderModal = ({ images, initialSlideIndex, isOpen, onClose }) => {
  ...
  useEffect(() => {
    if (isOpen && swiperRef.current && swiperRef.current.swiper) {
      swiperRef.current.swiper.slideTo(initialSlideIndex || 0, 0); // Go to initial image without animation
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="fixed inset-0 w-screen h-screen max-w-full max-h-full p-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-[9999] border-none rounded-none overflow-hidden left-[50%] top-[50%]">
