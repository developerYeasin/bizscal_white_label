...
const ProductCardDefault = ({
  ...
  return (
    <Card className="group w-full shadow-none border-none rounded-none overflow-hidden hover:shadow-[0px_5px_14px_0px_#11111130] transition-shadow flex flex-col">
      <Link to={getPath(`/products/${product.id}`)}>
        <div className="relative overflow-hidden">
          <img
            ...
            alt={product.name}
            className={cn(
              "w-full h-64 object-cover transition-opacity duration-500 ease-in-out",
          ...
          {hasHoverImage && (
            <img
              ...
              className="w-full h-64 object-cover absolute top-0 left-0 transition-opacity duration-500 ease-in-out opacity-0 group-hover:opacity-100"
            />
          )}
      ...
      <CardContent className="p-4 flex-grow">
        <Link to={getPath(`/products/${product.id}`)}>
          <h3
            className="text-lg font-semibold mb-2 text-foreground hover:text-dynamic-primary-color transition-colors line-clamp-2"
